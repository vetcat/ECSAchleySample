...
namespace anygames.ashley.utils
{
	...
	public class Bits
	{
		...
		{
			checkCapacity ( rightMove(nbits, 6));
		}
		...
		{
			long [] bits = this.bits;
			int word = rightMove (fromIndex, 6);
		...
		//		for (int i = commonWords, bits.Length; s > i; i++) {
		//			bits [i] = 0L;
		//		}
		//	}
		//}
		...
		{
			int word = rightMove (length (), 6);
			int hash = 0;
			for (int i = 0; word >= i; i++) {

				int t = rightMove ((int)bits [i], 32);

				hash = 127 * hash + (int)(bits [i] ^ ( t));
			}
			return hash;
		}
